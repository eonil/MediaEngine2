...
Vector3::Utility::angleBetweenVectorsOnPlane(const Eonil::Improvisations::MediaEngine::Mathematics::Geometry::Vector3 a, const Eonil::Improvisations::MediaEngine::Mathematics::Geometry::Vector3 b, const Eonil::Improvisations::MediaEngine::Mathematics::Geometry::Vector3 planeAxis)
{
	if (USE_EXCEPTIONS)
	{
		error_if(not almost_normalized(a, large_epsilon()), "All input vectors must be normalized.");
		error_if(not almost_normalized(b, large_epsilon()), "All input vectors must be normalized.");
		error_if(not almost_normalized(planeAxis, large_epsilon()), "All input vectors must be normalized.");
	}
