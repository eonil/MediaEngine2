...
#ifndef __Mathematics__Matrix__
#define __Mathematics__Matrix__
...
//					static M const		rotationByAngleAroundZAxis(Vector3 const angle);
		...
		Matrix4 : public SimpleMatrixAbstraction<4, Matrix4>
		{
			 ...
			 */
			Vector3 const	transform(Vector3 const point) const;
			Vector4 const	transform(Vector4 const vector) const;
			Vector2 const	transform(Vector2 const point) const;
...
//					static Matrix4 const	observation(Vector3 const eye, Vector3 const center, Vector3 const up);																	//	Same with `gluLookAt`. Creates a transform matrix for a camera.
		...
		static_assert(sizeof(Matrix4) == sizeof(Scalar) * 16, "");
...
#endif /* defined(__Mathematics__Matrix__) */
