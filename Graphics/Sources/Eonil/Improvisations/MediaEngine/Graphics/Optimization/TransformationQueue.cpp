...
namespace Eonil { namespace Improvisations { namespace MediaEngine { namespace Graphics {
	...
	Optimization
	{
		...
		{
			static inline void
			resetAllAsScales(Vector3 const* const __restrict__ scales, Matrix4* const __restrict__ transforms, Size const count)
			{
			...
			static inline void
			applyAllRotations(Quaternion const* const __restrict__ rotations, Matrix4* const __restrict__ transforms, Size const count)
			{
				for (Size i=0; i<count; i++)
				{
			...
			static inline void
			applyAllTransforms(Vector3 const* const __restrict__ translations, Matrix4* const __restrict__ transforms, Size const count)
			{
				for (Size i=0; i<count; i++)
				{
		...
		TransformationQueue::TransformationQueue(Size const count)
		{
			resize(count);
		}
		...
		void
		TransformationQueue::resize(const Size count)
		{
			if (_c != count)
			{
				delete[]	_ss;
