...
//				dfactor == GL_SRC_ALPHA_SATURATE			//	Not supported as destination.
	...
	eeglGetString(GLenum const name)
	{
		...
		{
			err9_converted_legacy_assertion(contains_target_in_samples(name,
													  {
														  GL_VENDOR,
														  GL_RENDERER,
														  GL_VERSION,
		...
		if (USE_DEBUGGING_ASSERTIONS)
		{
			err9_converted_legacy_assertion(c_str1 != NULL);		//!	Zero means error. @ref https://www.khronos.org/opengles/sdk/docs/man/xhtml/glGetString.xml
		}
