	...
	Utility
	{
		...
		namespace
		{
			...
			setAllChannelings(GenericMemoryRange<VertexComponentProvisioning<P> const> providers) -> void
			{
				for (auto const& provider: providers)
				{
					...
					{
						err9_converted_legacy_assertion(src_layout.channelComponents().size() == channeling.destinationSlots().size());
					}
					////
					
					sz	num_comps	=	src_layout.channelComponents().size();
					for (sz i=0; i<num_comps; i++)
					{
			...
			unsetAllChannelings(GenericMemoryRange<VertexComponentProvisioning<P> const> providers) -> void
			{
				for (auto const& provider: providers)
				{
					sz	num_comps	=	provider.channeling().destinationSlots().size();
					for (sz i=0; i<num_comps; i++)
					{
		...
		namespace
		{
			...
			_generic_draw(GeometryProvisioning const& vertexes, TextureChannelingRangePointer const& textures, DrawingMode const& mode, IndexingRange const& selection) -> void
			{
				if (USE_DEBUGGING_ASSERTIONS)
				{
					err9_converted_legacy_assertion(not _has_duplicated_generic_vidx(vertexes));
				}
...
//					}
		...
		draw(GeometryProvisioning const& vertexes, TextureChannelingRangePointer const& textures, DrawingMode const& mode, IndexingRange const& selection) -> void
		{
			if (USE_DEBUGGING_ASSERTIONS)
			{
				err9_converted_legacy_assertion(not vertexes.empty());
				err9_converted_legacy_assertion(not textures.empty());
				err9_converted_legacy_assertion(not selection.empty());
			}
