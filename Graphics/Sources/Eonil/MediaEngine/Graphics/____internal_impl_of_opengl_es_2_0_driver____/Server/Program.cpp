...
Server
{
	...
	_search_uniform_location(const str &name) const -> GLint
	{
		...
		err1_recoverable_bad_input_parameter_always("Uniform value slot for the name does not exist.");
	}
	auto
	Program::
	...
	{
		ObjectInstanceAddressTracker<Program>::registerObjectAddress(this);
...
//			printf("Program deleted: %u\n", name);
		...
		ObjectInstanceAddressTracker<Program>::unregisterObjectAddress(this);
	}
	...
	auto Program::
	operator==(const Program &other) const -> bool
	{
