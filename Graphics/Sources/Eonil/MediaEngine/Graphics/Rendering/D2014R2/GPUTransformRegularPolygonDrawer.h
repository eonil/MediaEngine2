...
GPUTransformRegularPolygonDrawer
{
	...
	static auto	_maximumCapacityOfCurrentPlatformForVaryingInstances() -> sz;			//!	Very expensive. (reads back from remote GL server)
...
public:
	GPUTransformRegularPolygonDrawer(sz const& segmentation);
	/*!
	 ...
	 */
	GPUTransformRegularPolygonDrawer(sz const& segmentation, sz const& capacity);
	~GPUTransformRegularPolygonDrawer();
