...
//				return	Matrix4::Utility::scaleWithVector(scale2);
			...
			Vector4 const
			_Regulator_v2::projectPoint(const Eonil::Improvisations::MediaEngine::Mathematics::Geometry::Vector4 pointInRSS) const
			{
				return	aspectRatioCompensationTransformMatrix().transform(pointInRSS);
			}
			Vector4 const
			_Regulator_v2::unprojectPoint(const Eonil::Improvisations::MediaEngine::Mathematics::Geometry::Vector4 pointInNDC) const
			{
				return	aspectRatioCompensationTransformMatrix().inversion().transform(pointInNDC);
			}
			
			Bounds2 const
			_Regulator_v2::projectBounds(const Eonil::Improvisations::MediaEngine::Mathematics::Geometry::Bounds2 boundsInRSS) const
			{
				return	boundsInRSS.boundsWithTransformationByMatrix(aspectRatioCompensationTransformMatrix());
			}
			Bounds2 const
			_Regulator_v2::unprojectBounds(const Eonil::Improvisations::MediaEngine::Mathematics::Geometry::Bounds2 boundsInNDC) const
			{
